...
/**
 * 关于 Paimon Primary Key Table Changelog Producer Is Input 说明
 * 1 会产生完整的 changelog
 * 比如比如在第一个 checkpoint 过程中 Table 输入 70 下一次 checkpoint 输入 80
 * 那么输出如下：
 * +I 80
 * 此时在表存储目录下生成一个 changelog-xxx-0.orc 文件(对应的数据文件 data-xxx-1.orc 保存数据 80) 里面保存了当前 checkpoint 的 changelog 的数据 比如 80
 * 第三次 checkpoint 输入 90 100
 * 那么输出如下：
 * +I 90
 * +I 100
 * 此时在表存储目录下生成一个 changelog-yyy-0.orc 文件(对应的数据文件 data-yyy-1.orc 保存数据 100) 里面保存了当前 checkpoint 的 changelog 的数据 比如 90 100
 * <p>
 * 如果二次 Job 重启不依赖于 savepoint 那么持续查询得到的结果为全量数据 ( primary key 合并后的数据)以及后续上游表快照产生的 changelog
 * <p>
 * 使用场景：
 * 1 抓取数据库表数据 CDC
 * 2 Flink 状态计算产生 changelog
 */
public class PrimaryKeyTableWithChangelogProduce_Input {
    ...
    public static void main(String[] args) {
        ...
        tableEnv.executeSql("DROP TABLE ods_kafka_source_hp_series_input");
        tableEnv.executeSql("CREATE TABLE IF NOT EXISTS ods_kafka_source_hp_series_input (\n" +
                "  `factory_num` STRING,\n" +
                "  `equ_type` STRING,\n" +
                "  `equ_num` STRING,\n" +
                "  `work_sta` STRING,\n" +
        ...
        tableEnv.executeSql("INSERT INTO ods_kafka_source_hp_series_input SELECT \n" +
                "\n" +
