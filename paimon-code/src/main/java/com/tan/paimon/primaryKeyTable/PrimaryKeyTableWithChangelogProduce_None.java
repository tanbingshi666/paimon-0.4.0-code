...
/**
 * 关于 Paimon Primary Key Table Changelog Producer Is None 说明
 * 1 不会产生 changelog (默认情况)
 * 比如在第一个 checkpoint 过程中 Table 输入 100 下一次 checkpoint 输入 110 、120
 * 那么输出 如下：
 * -U 100
 * +U 120
 * 对于下游持续查询 Job 而言 110 的 changelog 不会产生 也不会记录在 Paimon 表存储目录下
 */
public class PrimaryKeyTableWithChangelogProduce_None {
    ...
    public static void main(String[] args) {
        ...
        tableEnv.executeSql("CREATE TABLE IF NOT EXISTS ods_kafka_source_hp_series_none (\n" +
                "  `factory_num` STRING,\n" +
                "  `equ_type` STRING,\n" +
                "  `equ_num` STRING,\n" +
                "  `work_sta` STRING,\n" +
        ...
        tableEnv.executeSql("INSERT INTO ods_kafka_source_hp_series_none SELECT \n" +
                "\n" +
